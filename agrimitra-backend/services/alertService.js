...
async function createAlert(deviceId, type, severity, message) {
  ...
  const alert = new Alert({ deviceId, type, severity, message });
  await alert.save();
...
async function checkSensorAlerts(deviceId, reading) {
...
async function checkWeatherAlerts(deviceId, irrigationRec) {
